...
RSpec.describe 'check answers module' do
  ...
  context 'when sending match answer and question' do
    it 'returns true' do
      expect(subject.check_answer_from_module("test2", "test2", user_test2, "??", "false")).to include("true", @second_memo.id.to_s)
    end
  end

  context 'when sending non match answer and question' do
    it 'returns false' do
      expect(subject.check_answer_from_module("test3", "test2", user_test2, "??", "false")).to include("false", @second_memo.id.to_s)
    end
  end
